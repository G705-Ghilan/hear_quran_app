...
class LockCachingAudioSource extends StreamAudioSource {
  ...
  Future<HttpClientResponse> _fetch(
      HttpClientRequest httpRequest, HttpClient httpClient) async {
    _downloading = true;
    final cacheFile = await this.cacheFile;
  ...
  Future<StreamAudioResponse> request([int? start, int? end]) async {
    ...
    // mine
    // make the audio download only when it selected
    ...
    if (index == _player?.currentIndex) {
      try {
        ...
        final HttpClientRequest httpRequest =
            await _getUrl(httpClient, uri, headers: headers);
        _response ??= _fetch(httpRequest, httpClient)
